...
Then the finale: one SQL statement that walks the co-order graph from Lab 6, joins the relational truth from Lab 4, and ranks by vector distance — planned as **one tree by one optimizer**. That is the difference between a multi-model checkbox and a converged guarantee.
...
* Completed **Lab 6** — the order graph built and `MENU_MODEL` loaded
...
1. In the **SQL worksheet** — verify the model, add the column, embed:
    ...
    **What you should see:** `MENU_MODEL` (loaded in Lab 6), then the column added, then 7 rows updated. No embedding service, no API key, no copy pipeline — the vector is generated where the row already lives - in the database.

    ![Generate vector in the database](images/generate-vectors.png "Generate vector in the database")


    > **If `MENU_MODEL` is missing:** go back to **Lab 6, Task 1** and run that block again — it is idempotent, so re-running is free. 
...
1. Ask for something no menu item contains as keywords:
    ...
    **Why there is no index here, and when you would add one.** What you just ran is an *exact* search: the database compared your query vector against every row's vector and sorted by distance. On a seven-item menu that is the right answer and it is instant — an index would be pure overhead.
    ...
    The catch is in the word *approximate*: you trade a small, tunable chance of missing a true nearest neighbor for an enormous speed-up. You opt into that trade explicitly with `FETCH APPROX`:
    ...
    Don't run those here — at this scale the index would not be picked up since it makes the query slower, not faster. The point is that the *statement barely changes*: same table, same `WHERE`, same optimizer. Scaling this to a million menu items is an index decision, not a re-architecture.

## Task 3: The Freshness Proof — Insert, Then Search

1. This is the beat a bolt-on vector pipeline cannot replicate. Insert a brand-new item, embedding it **in the same transaction**, then immediately search. Run the following statements in your **SQL Worksheet**:
    ...
    Now let's see whether this newly added item actually makes a difference in the vector search, now for a more beefy choice.
...
1. The finale asks a question a real kiosk would ask: *given what this customer actually eats, and given a preference in plain English, what should we put in front of them?* Answering it needs all three models at once — and here is how the statement gets there, before you run it:
    ...
    * **The `myGraph` subquery is the graph step.** It walks `customer → placed → order → contains → item` for customer `c_1` and collects the distinct items they have actually ordered. That is their taste profile, derived from the order documents mongosh wrote — not a profile table somebody had to maintain.
    ...
    **What you should see:** the three items customer `c_5` actually orders, ranked by meaning — **Garden Salad first**.
    ...
    ![The one statement finale](images/the-finale.png "The one statement finale")
...
2. Before you read your first plan, thirty seconds of anatomy:
    ...
    ![How to read a plan in 30 seconds](images/plan-anatomy.png "Plan anatomy")
...
3. Now look at how the engine ran it:
    ...
    **What you should see:** one plan tree. Use the annotated expected-plan figure to find three things in yours: **(1)** the scans of the graph tables (`ORDER_ITEM` / `ORD` — that's the GRAPH_TABLE match), **(2)** the access of the canonical `ITEM` table (RELATIONAL), **(3)** the `SORT ORDER BY STOPKEY` at the top — the vector top-k (VECTOR). Exact rows vary with statistics; the three elements in a single tree do not. 
    
    Three models, row sources in **one tree**, planned by **one optimizer**, over data a Mongo driver wrote.

    ![Annotated expected plan — graph, relational, and vector in one tree](images/capstone-plan.png "Annotated expected plan")
...
* [AI Vector Search users guide](https://docs.oracle.com/en/database/oracle/oracle-database/26/vecse/)
