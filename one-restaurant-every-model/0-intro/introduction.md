...
- **Act I — The Bet.** You build the classic document-world model: one embedded menu document per store, using the MongoDB API against Oracle Autonomous AI Database, leveraging native JSON collections. The 90% (point reads, object-shaped data) feels great — until one corporate price change rewrites the whole fleet, an analytics question needs a triple-`$unwind` pipeline, and one drifted copy of the data quietly stops being true.
- **Act II — The Truth.** You discover the JSON collection you created using the MongoDB API **is already a SQL-queryable table** — same bytes, no import, no CDC — and shred it into a canonical relational schema where the drift is structurally impossible.
...
* Name the access-surface options on Oracle AI Database — MongoDB API, SQL and SQL/JSON, JSON Relational Duality Views, REST, property graph (SQL/PGQ), and AI Vector Search — and choose between them
