...
SET    s.data = JSON_TRANSFORM(s.data,
         SET '$.menus[*].categories[*].items[*]?(@.item_id.type() == "number" && @.item_id == 1000).price' = 13.99
         IGNORE ON MISSING);
COMMIT;

-- STATE CHECK: four stores at 13.99; s_104's drifted string copy untouched
-- at its local premium of 14.99
