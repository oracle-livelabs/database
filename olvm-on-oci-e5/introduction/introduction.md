# Oracle Linux Virtualization Manager on OCI - Beginner

## Introduction

## About this Workshop

Oracle Linux Virtualization Manager provides Oracle Linux KVM-based virtualization and centralized management for Oracle Linux environments. In this beginner workshop, you will deploy Oracle Linux Virtualization Manager on Oracle Cloud Infrastructure (OCI), configure a two-host Oracle Linux KVM cluster, set up networking and storage, deploy a multi-tier application, and perform live migration.
...
2. **Lab 2: Deploy Oracle Linux Virtualization Manager Engine** - Connect to the manager via SSH, install the Oracle Linux Virtualization Manager engine, and validate portal access.
3. **Lab 3: Configure Oracle Linux KVM Cluster** - Add `olkvm01` and `olkvm02` to the default cluster and wait for both hosts to reach `Up`.
...
**End Result:** A working Oracle Linux Virtualization Manager deployment on OCI with a two-host Oracle Linux KVM cluster, shared storage, multiple virtual machines, and a running Employee Directory application.
...
- Do not start Lab 4 until both Oracle Linux KVM hosts in Lab 3 show status `Up`.
- Do not start Lab 5 until Lab 4 confirms the logical network, Oracle Linux KVM host `l2-vm-network` addresses, storage domain, template import, and test VM are all working.
...
- In the E5 topology, guest VMs on `l2-vm-network` are accessed through the Oracle Linux KVM host shown in the VM **Host** column. The `olvm` manager controls the VMs, but it is not used as the direct network path to guest VM IP addresses.
- In Lab 5, keep the Employee Directory database VM and web application VM on the same Oracle Linux KVM host. Cross-host application networking is not part of the beginner path.

### Objectives

In this workshop, you will:

- Deploy Oracle Linux Virtualization Manager Engine and verify portal access
- Add and configure Oracle Linux KVM hosts in a cluster
- Configure logical networking for virtual machines
- Verify the OCI and Oracle Linux Virtualization Manager network paths used by the manager, Oracle Linux KVM hosts, and virtual machines
...
The Oracle Linux Virtualization Manager hosts are provisioned with `VM.Standard.E5.Flex` shapes:
...
This workshop creates a VLAN inside an OCI Virtual Cloud Network (VCN). The VLAN provides the Layer 2 network used by the virtual machines that run on the Oracle Linux KVM hosts.

Before workshop day, confirm with your tenancy administrator that VLAN / Layer 2 network virtualization is available in the target OCI region and compartment. Lab 1 includes this check before the Oracle Linux Virtualization Manager labs begin.
...
**Oracle Linux Virtualization Manager** is an open-source, enterprise-grade virtualization management platform for Oracle Linux Oracle Linux KVM environments. It provides a centralized web-based portal for deploying, monitoring, and managing Oracle Linux KVM-based virtual machines across one or more hypervisor hosts. Oracle Linux Virtualization Manager is supported by Oracle and integrates with Oracle Linux.

**Oracle Linux KVM, or Kernel-based Virtual Machine,** is the hypervisor built into the Linux kernel. It provides the virtualization layer that allows Oracle Linux hosts to run virtual machines.

**Oracle Cloud Infrastructure (OCI)** is Oracle's cloud platform. In this workshop, OCI provides the compute instances, networking, and storage resources used to host the Oracle Linux Virtualization Manager environment.
...
**OCI VLANs** provide the Layer 2 network used by guest virtual machines. This network allows VMs running on the Oracle Linux KVM hosts to communicate through the workshop's virtualized network path.

   ![Oracle Linux Virtualization Manager on OCI workshop topology](./images/olvm-oci-workshop-topology.png "Oracle Linux Virtualization Manager on OCI workshop topology")
   *Workshop topology for Oracle Linux Virtualization Manager on OCI, including the Oracle Linux Virtualization Manager manager, two Oracle Linux KVM hosts, guest VMs, OCI VLAN-based Layer 2 VM networking, and shared storage.*
