# Find the Closest Service Center for Residential Customers

## Introduction

Moon Kai is the Spatial and Geo Specialist on Jessica Chan's State and Local Government team. Operations teams ask Moon for help when location affects a service decision: which public-service center is closest to a region with growing demand, and which residential customers should it handle?

The team already stores the required data in Oracle AI Database. Public-service centers and residential customers are stored as map points. Demand regions are stored as map areas, and each region has a demand score.

Moon wants operations users to answer a simple question with SQL that can power a business-user dashboard and map:

> A region needs more service support. **Which residential customers are in that region, and which public-service center is closest to each one?**

In this lab, you follow Moon's approach. You start with a single point, measure distance to a region, find residential customers inside that region, and finish with a routing result that combines location and public-service data.
...
> - A **point** is one location, represented by longitude and latitude. In this lab, a public-service center or residential customer is stored as an `SDO_GEOMETRY` point.
>
...
>
> - A **spatial relationship** describes how two shapes relate to each other. `SDO_GEOM.RELATE` can test whether a residential customer point is inside or touches a demand region.
>
...
The State and Local Government LiveStack uses the same data in its public-service coverage view. The map helps users see the result; the SQL in this lab shows how Oracle calculates it.
...
- Find residential customers inside a demand region.
- Match each residential customer to the closest active service center.
...
| Technical Challenge | Moon needs to compare residential customer points with a region, then find the closest center for each residential customer. |
| Persona Focus | You review Moon's spatial approach and interpret the result for an operations user. |
| What You Will See | Oracle Spatial turns location data into residential customer routing results with SQL. |
| Database Capability | `SDO_GEOMETRY`, `SDO_GEOM.SDO_DISTANCE`, `SDO_GEOM.RELATE`, and GeoJSON conversion support the analysis. |
| Outcome | An operations user can see which residential customers need service in a region and which center is closest to each one. |
...
An `SDO_GEOMETRY` point is Oracle Spatial's structured representation of one location. A public-service center or residential customer is stored with a point type, a WGS84 coordinate system, and a longitude-and-latitude pair. Because Oracle stores the location as geometry, Spatial functions can calculate distance and test spatial relationships instead of treating the coordinates as two unrelated numbers.
...
The Western Slope is a useful region for the first routing review. Moon now measures the distance from each public-service center point to the region boundary.
...
1. Run the distance query:
    ...
    **Expected output: Western Slope Service Coverage**
    ...
    The result ranks the public-service centers by distance to the Western Slope demand region and includes the region's demand score. Use the closest center as the starting point for the capacity review.

    ![result](images/task21.png)

2. Try another region.

    Change the region name to `Front Range`, add a miles calculation, and run the modified query:
    ...
    The `unit` parameter controls the measurement unit. The result may change because the query is measuring against a different public-service region. Compare the kilometer and mile columns rather than treating the distance as an assignment by itself.

    This is a useful regional result, but distance to the region boundary does not identify the residential customers who need service. Moon now uses the region polygon to find those residential customers and then assigns each one to the closest active center.

3. Compare regional service capacity.

    Distance alone is not enough. Use the public-service capacity view to compare available and reserved workload with the highest center utilization in each region.
    ...
    **Expected output: Regional Public-Service Capacity**
    ...
    The result returns one row per service region with available workload, reserved workload, and the highest center utilization. Moon can use these values with the distance results to identify a practical center for further review.

    ![SQL Worksheet result showing regional public-service capacity](images/regional-public-service-capacity.png " ")

## Task 3: Route residential customers to the closest center

Moon now needs a result that a public-service operations application can use: residential customers inside the Western Slope, their demand region, and the closest active service center. The query uses the customer point (**`c.location`**) and demand-region polygon (**`dr.boundary`**) to find the residential customers first. It then compares each customer point with every active center and keeps the closest one.

1. Run the residential customer routing query:
    ...
    `SDO_GEOM.RELATE` keeps residential customers whose point falls inside or touches the Western Slope polygon. `SDO_GEOM.SDO_DISTANCE` then measures the distance from each matching residential customer to every active center. `ROW_NUMBER` keeps the nearest center for each residential customer.

2. Review the result as an operations decision.

    Each row gives an operations user a residential customer, the closest center, and the information needed to decide where the service work should go. The result combines the region's demand score, customer details, center capacity, current load, and spatial distance in one SQL result.

    This is the business outcome. A dashboard can let a user select a region and immediately show the residential customers affected, the center that should handle each request, and the distance involved. The user does not need to compare a map with a separate customer list or operations report.

    ![SQL Worksheet result showing residential customers routed to nearby service centers](images/task32.png)

3. Change the query to `Front Range`.

    Compare the residential customers and assigned centers with the Western Slope result. The spatial predicates stay the same; only the region changes. This is the kind of query a public-service operations dashboard can run when a user selects a different demand region.

    ![result](images/task31.png)

## Conclusion: Turn Location into a Service Decision

Moon's analysis moves from a point, to distance, to residential customer routing. An operations user can select a high-demand region and get a list of residential customers, their closest service center, and the distance to that center. That is a useful dashboard result because it tells the user what action to review, not just where the data is located.

This shows why Spatial in Oracle AI Database matters. One convergent query can identify residential customers with spatial functions, join them to relational customer and center data, and include capacity and current load in the same result. The dashboard can show the map and the business details from one database, without moving data between a mapping system, a customer system, and a public-service operations system.
