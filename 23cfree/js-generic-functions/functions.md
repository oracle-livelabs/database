...
After creating JavaScript modules and environments in the previous lab you are now learning how to expose JavaScript code in SQL and PL/SQL. After completing this step you can call JavaScript code anywhere you can invoke SQL and PL/SQL functions. All client code,regardless whether it's written in Java, Python, or even with `node-oracledb`, should be able to access JavaScript store procedures.
...
- Followed Lab 2 where you created a number of JavaScript modules in the database

## Task 1: Learn more about call specifications

Call specifications are a standard extension to the PL/SQL language allowing you to provide instructions in a different programming language. Typically you would provide the function or procedure body in PL/SQL except if you want to link it to Java or JavaScript code.
...
![PL/SQL call specification clause in CREATE PROCEDURE statement](images/001-functions.jpg)

In case of JavaScript developers can choose from 2 options:

- Refer to a function in a module
- Provide the JavaScript code inline with the PL/SQL code unit

![JavaScript call specification](images/002-functions.jpg)

Both of these will be addressed in this lab.
...
In this task you learn how to create a call specification based on the MLE module name and (JavaScript) function.

1. Review the source code for `helper_module_inline`

	Before you can export functions defined in a module you need to ensure they are `exported`. JavaScript in Oracle Database 23c Free-Developer Release uses ECMAScript modules. 
...
2. Create call specification for `helper_module_inline`
	...
	A PL/SQL function is the best fit. You can either create stand-alone functions like you did in lab 1, however in this case it is best to group the functions in a PL/SQL package. Using the syntax diagrams from step 1 you can create the call specification as follows:
...
Before you can create a call specification for `processOrder()` you must ensure an MLE environment exists mapping the import name `helpers` to the `helpers_module_online`.
...
3. Invoke the JavaScript code
	...
	Just like in task 3 you can now invoke the code thanks to the call specificiation you just created.
	...
	begin
		...
		-- you should probably think of a better success/failure evalution
		if l_success then
...
1. Hello World example using inline JavaScript code
	...
	> **Note** JavaScript identifiers are case sensitive and therefore have to be enclosed in double-quotes in the PL/SQL layer or else they wouldn't be recognised in the JavaScript portion of the code.
...
2. Understand the query output
	...
	The above query will return output similar to the following
	...
	The way the view is defined you will sometimes see both `object_name` and `procedure_name` populated, and sometimes it's just `object_name` whilst `procedure_name` is null. 
...
- Chapter 5 in [JavaScript Developer's Guide](https://docs.oracle.com/en/database/oracle/oracle-database/23/mlejs/mle-js-modules-and-environments.html#GUID-32E2D1BB-37A0-4BA8-AD29-C967A8CA0CE1) describes call specifications and inline JavaScript functions in detail
