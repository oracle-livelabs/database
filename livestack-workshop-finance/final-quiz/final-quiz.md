...
1. Complete the scored quiz.
    ...
    - To teach learners how to manually install every finance table.
    * To map the shared data used by each later finance workflow.
    ...
    > The foundation lab orients you to the shared database objects behind the application. Later labs reuse that foundation for dashboard evidence, vector search, graph analysis, spatial coverage, OML scoring, governed answers, and agent actions.
    ...
    * It explains connections across accounts and shared entities.
    ...
    - It replaces audit history with graph-only output.
    ...
    * Predictions can be scored where governed data already lives.
    ...
    - They use hidden prompts and untracked action history.
    - They query unapproved tables without visible SQL or audit evidence.
    * They use approved views, visible SQL, tools, and audit rows.
    - They return browser-only answers that cannot be repeated.
