...
Throughout the workshop, expandable sections provide optional background on key terms, concepts, and **Oracle Database** capabilities. Select the arrow when you want extra context. These sections are closed by default so the main lab stays focused, but they are useful if a concept is new or if you want to explain it to another stakeholder.
...
>
> In a fractured environment, each capability often lives in a different store or service.
>
> That can force teams to copy data, rebuild security rules, reconcile conflicting results, and explain why two systems disagree. Oracle Database is well suited for this finance scenario because the evidence, security model, SQL access, and application data stay connected.

</details>

The hands-on work follows one finance decision flow. First, orient yourself to the data foundation. Then use that foundation to explain dashboard evidence, inspect transaction documents, search risk language by meaning, follow fraud relationships, evaluate service coverage, and score predictive models.

Each lab starts with a practical finance question, then shows the SQL evidence that supports the answer.
...
The image below shows the **Seer Bank Finance LiveStack** welcome page. It frames the workshop as one connected financial-intelligence journey: monitor risk, investigate exposure, route work, analyze capacity, and use predictive evidence. The SQL labs then show the database evidence behind that application journey.
...
**Persona focus:** You support finance business users who need timely, explainable decisions without fragmented integration work. Your job is to connect each business decision to governed database evidence that can be reviewed and repeated.
