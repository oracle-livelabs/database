...
At Big Star Collectibles, a inventory specialist asked the AI assistant about rates for preferred customers:

> *"I asked the AI what rates we offer preferred customers. It said 6.5%. Our actual platinum loyalty pricing is 7.9%. I almost quoted wrong rates to a client!"*
>
> Marcus, Senior Inventory Specialist

The chatbot doesn't know Big Star Collectibles' actual rates, policies, or client information. It gives generic answers that sound confident but are confidently wrong.
...
**What you'll build:** An agent connected to Big Star Collectibles' authentication policies and collector data.

Estimated Time: 10 minutes

### Story Sync
**Story Sync:** Chapter 2.2 – see the corresponding narrative beat for context.
...
1. Set the AI profile and ask about Big Star Collectibles' rates.
    ...
    SELECT AI CHAT What pricing tiers does Big Star Collectibles offer for preferred customers;
    </copy>
    ```
...
2. Ask about a specific collector.
    ...
    ```sql
    <copy>
    SELECT AI CHAT Is collector Alex Martinez eligible for platinum loyalty pricings at Big Star Collectibles;
    </copy>
    ```
...
1. Create authentication policy and collector tables.

    These tables contain Big Star Collectibles' actual business information: real rate tiers, real credit requirements, and real client data. This is what turns a generic AI into YOUR AI.

    > This command is already in your notebook - just click the play button (▶) to run it.

    ```sql
    <copy>
    -- Big Star Collectibles authentication policies
    ...
    INSERT INTO item_policies VALUES (
        ...
        'Minimum condition grade 550 for any item consideration. ' ||
        'Condition grade 750+ qualifies for Preferred tier. ' ||
        'Condition grade 650-749 qualifies for Standard tier. ' ||
        'Condition grade below 650 requires additional documentation and cosigner.',
        'ALL',
        'All item types'
    );

    -- Big Star Collectibles collector data
    CREATE TABLE se_collectors (
    ...
    INSERT INTO se_collectors VALUES ('CUST-001', 'Alex Martinez', 'Platinum Club', 820, 'PLATINUM', DATE '2018-02-11', 20, 9);
    INSERT INTO se_collectors VALUES ('APP-002', 'TechStart LLC', NULL, 710, 'STANDARD', DATE '2022-06-01', NULL, 2);
    INSERT INTO se_collectors VALUES ('APP-003', 'GlobalCo', NULL, 620, 'STANDARD', DATE '2024-01-10', NULL, 1);
...
2. Create tool functions to access this data.
    ...
    ```sql
    <copy>
    -- Tool to look up authentication policies
    ...
    -- Tool to look up collector
    CREATE OR REPLACE FUNCTION get_collector_info(
        p_collector_name VARCHAR2
    ) RETURN VARCHAR2 AS
        v_result VARCHAR2(1000);
    BEGIN
        SELECT 'Collector: ' || name || 
               CASE WHEN company IS NOT NULL THEN ' (' || company || ')' ELSE '' END ||
               ', Condition Grade: ' || credit_score || 
               ', Rate Tier: ' || rate_tier ||
               ', Client Since: ' || TO_CHAR(client_since, 'YYYY-MM-DD') ||
               ', Total Items: ' || total_items ||
               CASE WHEN rate_exception IS NOT NULL 
                    THEN ', Loyalty Discount: ' || rate_exception || '% discount'
                    ELSE ', No loyalty discount' END
        INTO v_result
        FROM se_collectors
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN 'Collector not found: ' || p_collector_name;
        WHEN TOO_MANY_ROWS THEN
            RETURN 'Multiple collectors found matching: ' || p_collector_name || '. Please be more specific.';
    END;
    /
    </copy>
    ```
...
3. Register the tools.
    ...
    We turn these functions into tools the agent can use. The instructions tell the agent to ALWAYS use these tools for policy and collector questions - never guess. This is how you prevent the agent from making up answers.
    ...
    BEGIN
        DBMS_CLOUD_AI_AGENT.CREATE_TOOL(
            ...
            attributes  => '{"instruction": "Look up Big Star Collectibles authentication policies. Parameters: P_POLICY_TYPE (e.g. rate, credit, preferred, standard), P_RATE_TIER (PREFERRED or STANDARD, optional). Always use this to answer policy questions - never guess at rates or requirements.",
        ...
        DBMS_CLOUD_AI_AGENT.CREATE_TOOL(
            ...
            attributes  => '{"instruction": "Look up collector information. Parameter: P_COLLECTOR_NAME (full or partial name). Returns condition grade, rate tier, client history, and any loyalty discounts. Always use this when asked about specific collectors.",
                            "function": "get_collector_info"}',
            description => 'Retrieves collector details including credit tier and loyalty discounts'
        );
    END;
    /
    </copy>
...
Now let's create an agent with access to Big Star Collectibles' enterprise data. The key difference from a regular chatbot is that this agent has tools to look up real information.
...
1. Ask about rates for preferred customers.
    ...
    ```sql
    <copy>
    SELECT AI AGENT What pricing tiers does Big Star Collectibles offer for preferred customers;
    </copy>
    ```
...
2. Ask about Alex Martinez specifically.
    ...
    ```sql
    <copy>
    SELECT AI AGENT Is Alex Martinez eligible for platinum loyalty pricings and does she have any special pricing;
    </copy>
    ```
...
4. Ask a combination question.
    ...
    ```sql
    <copy>
    SELECT AI AGENT What rate would TechStart LLC qualify for on a new item;
    </copy>
    ```
...
**Key takeaway:** Agents don't fail because they're not smart. They fail because they don't know your business. Marcus almost quoted wrong rates because the AI didn't have access to Big Star Collectibles' actual pricing guides. Enterprise data is what transforms generic AI into your AI.
...
EXEC DBMS_CLOUD_AI_AGENT.DROP_TOOL('COLLECTOR_LOOKUP_TOOL', TRUE);
DROP TABLE item_policies PURGE;
DROP TABLE se_collectors PURGE;
DROP FUNCTION get_item_policy;
DROP FUNCTION get_collector_info;
</copy>
```
