...
>
> Jennifer, Senior Inventory Specialist
...
In this lab, you'll build **semantic search**, the ability to find relevant past appraisal decisions by *meaning*, not just keywords:
...
## Task 4: Populate Big Star Collectibles Authentication Policies

Add Big Star Collectibles' authentication policies. These are the corporate rules the agent must follow when processing item submissions.
...
2. Create functions to record and find appraisal decisions with semantic search.
    ...
    ```sql
    <copy>
    -- Record a appraisal decision (with semantic embedding)
...
3. Create a function to look up authentication policies.

    > This command is already in your notebook - just click the play button (▶) to run it.

    ```sql
    <copy>
    -- Look up authentication policies
...
BEGIN
    DBMS_CLOUD_AI_AGENT.CREATE_TOOL(
        ...
        attributes  => '{"instruction": "Record a appraisal decision for audit trail. Parameters: P_CLIENT (client name), P_SITUATION (what was the request/situation), P_DECISION (what was decided), P_OUTCOME (optional: what happened as a result). Use after making or discussing significant appraisal decisions.",
                        "function": "record_item_decision"}',
        description => 'Records appraisal decisions for audit and learning with semantic embeddings'
    );
END;
/
...
BEGIN
    DBMS_CLOUD_AI_AGENT.CREATE_TOOL(
        ...
        attributes  => '{"instruction": "Search for similar past appraisal decisions using semantic search. Parameter: P_SITUATION (describe the situation - the search finds similar meanings, not just keywords). Use when facing a decision to learn from past experience.",
...
BEGIN
    DBMS_CLOUD_AI_AGENT.CREATE_TOOL(
        ...
        attributes  => '{"instruction": "Look up Big Star Collectibles authentication policies. Parameters: P_POLICY_TYPE (optional: rate_policy, escalation_policy, exception_policy), P_SEARCH (optional: search term). Use when you need to know company collectibles policy or rates.",
                        "function": "lookup_policy"}',
        description => 'Retrieves Big Star Collectibles authentication policies and guidelines'
    );
END;
/
</copy>
...
Add historical appraisal decisions that the agent can learn from. Notice we're using varied wording - "seasonal cash flow," "cyclical revenue," "variable income" - to test semantic search.
...
INSERT INTO seers_memory (memory_type, entity_id, content, embedding) VALUES (
    ...
    '{"situation": "Long-term client requested loyalty discount for expansion item",
      ...
      "recorded": "2024-08-20"}',
    VECTOR_EMBEDDING(ALL_MINILM_L12_V2 USING 
        'Long-term client requested loyalty discount for expansion item. Approved 12% rate discount based on 7-year relationship and $2M in previous items. Client expanded successfully, referred two new business clients.' AS DATA)
);
...
INSERT INTO seers_memory (memory_type, entity_id, content, embedding) VALUES (
    ...
    '{"situation": "New business client requested loyalty discount without established relationship",
      ...
      "recorded": "2024-04-10"}',
    VECTOR_EMBEDDING(ALL_MINILM_L12_V2 USING 
        'New business client requested loyalty discount without established relationship. Denied exception request outright citing policy. Client went to competitor, later became successful business we lost.' AS DATA)
);
...
INSERT INTO seers_memory (memory_type, entity_id, content, embedding) VALUES (
    ...
    '{"situation": "New business client requested loyalty discount without established relationship",
      ...
      "outcome": "Client accepted, became loyal customer, now qualifies for platinum loyalty pricings",
      "recorded": "2024-05-22"}',
    VECTOR_EMBEDDING(ALL_MINILM_L12_V2 USING 
        'New business client requested loyalty discount without established relationship. Offered standard rate with commitment to review for exception after 12 months of on-time payments. Client accepted, became loyal customer, now qualifies for platinum loyalty pricings.' AS DATA)
);
...
1. Teach the agent about a client.
    ...
    ```sql
    <copy>
    SELECT AI AGENT Platinum Club is one of our best clients. They prefer email contact through their CFO Alex Martinez. They have been with Big Star Collectibles since 2019 and were approved for a 15% loyalty discount due to their excellent payment history on 4 previous items. Please remember all of this;
    </copy>
    ```
...
2. Ask about Platinum Club in the "new" session.

    > This command is already in your notebook - just click the play button (▶) to run it.

    ```sql
    <copy>
    SELECT AI AGENT What do you know about Platinum Club and their rate arrangements;
    </copy>
    ```
