...
> *"I asked about a customer's item status, and it told me how to log in. I know how to log in! I wanted the actual status."*
>
> Marcus, Senior Inventory Specialist

The chatbot was helpful for general questions like "What makes a comic book valuable?" or "How does grading work?" But when inventory specialists needed actual data or wanted to take action, it fell short.

Big Star Collectibles needs AI that can:
- **Access real data**: Query the actual item database
...
**What you will build:** A comparison demonstrating when to use chat vs agents, plus an agent that can both read AND update item data.
...
4. Paste the following GitHub URL leaving the credential field blank:
    ...
    ```text
    <copy>
    https://github.com/davidastart/database/blob/main/ai4u/industries/bigstar/agents-vs-zero-shot/lab2-agents-vs-zero-shot.json
    </copy>
    ```
...
2. Ask a procedural question using zero-shot.
    ...
    **Observe:** You get a helpful explanation of the steps. But you still have to do each step yourself.
    ...
    ```sql
    <copy>
    SELECT AI CHAT How do I authenticate a collectible item;
    </copy>
    ```
...
4. Ask it to do something.
    ...
    **Observe:** The AI explains HOW to update an item but cannot actually do it. Zero-shot can advise; it cannot act.

    > This command is already in your notebook - just click the play button (▶) to run it.

    ```sql
    <copy>
    SELECT AI CHAT Update item ITEM-12345 to graded;
    </copy>
    ```
...
1. Create a sample item submissions table with comments to help Select AI understand the schema.
    ...
    COMMENT ON TABLE sample_items IS 'Big Star Collectibles item submissions with authentication status.';
    COMMENT ON COLUMN sample_items.submission_id IS 'Unique item identifier. Examples: ITEM-12345, ITEM-12346.';
    COMMENT ON COLUMN sample_items.submitter IS 'Name of the person or business submitting the item';
    COMMENT ON COLUMN sample_items.status IS 'Submission status: SUBMITTED, AUTHENTICATING, GRADED, LISTED, or REJECTED';
    COMMENT ON COLUMN sample_items.value IS 'Estimated market value in US dollars';
    COMMENT ON COLUMN sample_items.item_type IS 'Type of collectible: Sports Card, Comic Book, Vintage Toy, or Memorabilia';
    COMMENT ON COLUMN sample_items.submission_date IS 'Date the item was submitted';

    -- Insert sample data
    INSERT INTO sample_items VALUES ('ITEM-12345', 'Acme Collectibles', 'AUTHENTICATING', 1500, 'Comic Book', SYSDATE - 3);
    INSERT INTO sample_items VALUES ('ITEM-12346', 'TechStart Cards', 'SUBMITTED', 450, 'Sports Card', SYSDATE - 1);
    INSERT INTO sample_items VALUES ('ITEM-12347', 'GlobalCo Toys', 'GRADED', 2750, 'Vintage Toy', SYSDATE - 7);
    COMMIT;
    </copy>
    ```
...
SELECT AI returned the actual status: `AUTHENTICATING`. Compare this to zero-shot which could only give generic advice.
...
4. Now try to update using SELECT AI.
    ...
    **Observe:** SELECT AI cannot update data. It can only read.
    ...
    ```sql
    <copy>
    SELECT AI NARRATE Update item ITEM-12345 to graded;
    </copy>
    ```
...
Still `AUTHENTICATING`. SELECT AI can read but cannot write.
...
1. Create a function to look up items (read).
    ...
    This function takes an item ID and returns the item details. It's a simple read operation that the agent will use to check on items before taking action.
...
2. Create a function to update item status (write).
    ...
    This function actually changes data in the database. Notice it first checks the current status, then makes the update. The agent will use this to grade or list items.
...
3. Register both functions as tools.
    ...
    BEGIN
        DBMS_CLOUD_AI_AGENT.CREATE_TOOL(
            ...
            description => 'Retrieves item status, submitter, value, and type by submission ID'
        );
    END;
    /
    ...
    BEGIN
        DBMS_CLOUD_AI_AGENT.CREATE_TOOL(
            ...
            attributes  => '{"instruction": "Update an item submission status. Parameters: P_SUBMISSION_ID (the item number), P_NEW_STATUS (SUBMITTED, AUTHENTICATING, GRADED, LISTED, or REJECTED). Only call this after confirming the current status with ITEM_LOOKUP_TOOL.",
...
4. Create the agent, task, and team.
    ...
    BEGIN
        DBMS_CLOUD_AI_AGENT.CREATE_AGENT(
            ...
            attributes  => '{"profile_name": "genai",
                            "role": "You are an item management agent for Big Star Collectibles. You can look up item submissions and update their status. Always look up an item first before updating it. Never make up item information - always use your tools."}',
    ...
    BEGIN
        DBMS_CLOUD_AI_AGENT.CREATE_TASK(
            ...
            attributes  => '{"instruction": "Help with item submission inquiries and updates. When asked to check an item, use ITEM_LOOKUP_TOOL. When asked to update an item, first use ITEM_LOOKUP_TOOL to verify current status, then use ITEM_UPDATE_TOOL to make the change. Do not ask clarifying questions - just do it. User request: {query}",
...
1. First, check the current status of item ITEM-12345.
    ...
    ```sql
    <copy>
    SELECT submission_id, submitter, status FROM sample_items WHERE submission_id = 'ITEM-12345';
    </copy>
    ```

    ![Task Information](./images/task5_1.png " ")

The item is currently `AUTHENTICATING`.
...
2. Set the team and ask the agent to check and update the item.
    ...
    SELECT AI AGENT Check item ITEM-12345 and if it is authenticating, grade it;
    </copy>
    ```
...
**Observe:** The agent:
1. Called `ITEM_LOOKUP_TOOL` to check current status (`AUTHENTICATING`)
2. Made a decision based on the result
3. Called `ITEM_UPDATE_TOOL` to change it to `GRADED`
...
**The status changed from `AUTHENTICATING` to `GRADED`.** The agent didn't just talk about updating - it actually did it.
...
4. Try a conditional update that should NOT happen.
    ...
    ```sql
    <copy>
    SELECT AI AGENT Check item ITEM-12346 and if it is authenticating, grade it;
    </copy>
    ```

    ![Task Information](./images/task5_4.png " ")

**Observe:** The agent looked up item ITEM-12346, saw it was `SUBMITTED` (not `AUTHENTICATING`), and correctly decided NOT to update it. This is intelligent coordination.
...
**Key takeaway:** The difference is not just intelligence. It's action. Zero-shot AI tells you what to do. SELECT AI can read. Agents do the work. For Big Star Collectibles, that means inventory specialists can focus on customers instead of data entry.
