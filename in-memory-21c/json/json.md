...
1. First let's verify that the database has been setup with the required prerequisites.
    ...
    ![Query output](images/Lab07-Step1.png)
...
1. First we will display the J_PURCHASEORDER table. This is the table we will be using for our examples and is the same table that is created and used in the 19c JSON Developer's Guide.
    ...
    ![Query output](images/Lab07-Step2.png)

2. The next step will define a check constraint on the PO_DOCUMENT column to ensure well formed JSON and enable In-Memory optimizations on the JSON data.
    ...
    ![Query output](images/Lab07-Step3.png)

    Note that not only have we created a check constraint on the PO_DOCUMENT column, but that has also resulted in the creation of a new virtual column that starts with the name SYS_IME_OSON. Oracle Database has created a special binary version of the PO_DOCUMENT column.
...
4. Next we will run a query on the JSON data to show the functionality and peformance. Remember that we have not yet enabled in-memory processing for the JSON data.
    ...
    ![Query output](images/Lab07-Step4-1.png)
    ![Query output](images/Lab07-Step4-2.png)

5. Now we will enable the J_PURCHASEORDER table for inmemory and populate it.
    ...
    ![Query output](images/Lab07-Step5.png)

6. Verify that the J_PURCHASEORDER table has been populated in the IM column store.
    ...
    ![Query output](images/Lab07-Step6.png)

7. Since a virtual column was created it will be stored in the IM column store as an In-Memory Expression (IME). See Lab 4 for details about IMEs. Let's check how much additional space the binary JSON data requires.
    ...
    ![Query output](images/Lab07-Step7.png)
...
8. Now let's run the JSON query again. This is the same query from step 3.
    ...
    ![Query output](images/Lab07-Step8-1.png)
    ![Query output](images/Lab07-Step8-2.png)

## Task 3: Populating JSON data in the IM column using the JSON data type in 21c

1. Now let's switch to a new table with the PO_DOCUMENT column defined as a JSON data type. Recall that this is new in Oracle Database 21c.
    ...
    ![Query output](images/Lab07-Step9.png)

    Note that no constraint definition is required.

2. Next we will populate the JSON_PURCHASEORDER table that we looked at in Step 8 with the JSON data type and we will remove the previous J_PURCHASEORDER table since we won't need it any longer.
    ...
    ![Query output](images/Lab07-Step10.png)

3. Make sure that the JSON_PURCHASEORDER table has been fully populated.
    ...
    ![Query output](images/Lab07-Step11.png)

4. Now we will query the JSON_PURCHASEORDER table with the same basic query that we used in Step 7.
    ...
    ![Query output](images/Lab07-Step12-1.png)
    ![Query output](images/Lab07-Step12-2.png)
...
5. (Optional) - You can optionally remove the JSON table(s) from the IM column store to save space and re-run this lab if you wish.
    ...
    Run the script *13\_unpop\_json.sql*
    ...
    ![Query output](images/Lab07-Step13.png)
