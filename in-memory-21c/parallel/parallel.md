...
5. Next we will run the same query as in the previous step but this will be a parallel query.
		...
		Notice that we enabled the session level parameter PARALLEL_DEGREE_POLICY of AUTO. This enabled parallel query and the optimizer chose a degree of parallelism of 2. We still accessed the LINEORDER table in-memory but we did it using parallel query.
...
10. Lastly, let's see how In-Memory Dynamic Scans, or IMDS, works with in-memory queries.
		...
		-- Since this is being done on a laptop, the following forces IMDS to be used
		--
