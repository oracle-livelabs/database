...
Write a query that returns the number of users in USA who have watched at least one episode of show 16 after 2021-04-01.
**Query 2 a:**
````
<copy>
select count(*) as cnt
...
**Example:** Write a query that returns the number of users who have watched at least one episode of show 15 after 2021-04-01.
...
````
In this example, you are forcing the use of index `idx_country_showid_date`.


4. Placing conditions on the elements of sibling arrays
You can place conditions on the elements of any arrays.

**Example:** Write a query that returns the number of users in USA who have watched a French or Danish
show in 2021.
...
**Indexes used**
The query uses the `idx_country_genre index`. The country and genres conditions are pushed to the index. Two index scans will be performed: one scanning the keys with value (`USA`, `french`) and another scanning the keys with value (`USA`,`danish`). The date conditions will be applied on the table rows associated with the qualifying index keys.
...
**Example:** Write a query that returns the user’s account, user ids, the total time the user has spent watching show 16, and an array containing information about all the episodes of show 16 that the user has watched. The users should have watched at least one episode of show 16 after 2021-4-01.
````
<copy>
...
**Example:** The query returns the number of users who have fully watched show 15 (all seasons and all episodes to their full length).
...
The query uses the index `idx_showId`, pushing the `showId` condition to it. The other conditions cannot be pushed to any index.
...
**Example:** For each show, write a query that returns the number of users who have shown an interest in that show. Sort the results by the number of users in descending order.
...
**Example:** For each show, return the total time users have spent watching that show. Sort the results in descending order of the total time.
...
**Example:** For each show and associated season, return the total time users have spent watching that show and season. Sort the results in descending order by the total time.
