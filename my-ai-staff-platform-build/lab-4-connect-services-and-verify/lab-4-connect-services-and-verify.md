...
1. Configure Google Drive through rclone only when Drive delivery is required. The platform produces local files when Drive is disabled.
2. Configure Gmail and Google Calendar OAuth only when AI Staff email and calendar features are required. Current email support is Gmail-only; do not substitute an Outlook or IMAP configuration.
3. Configure OCI Email Delivery only when pause-reminder email is required.
4. Validate each optional integration independently before moving to full verification.
...
3. Validate database health from the Data Agent endpoint.
    ...
    curl -sm 10 https://editor.<your-domain>/health
    </copy>
    ```

4. If database checks fail, revisit the wallet, `AI_FOR_YOU` schema, and `MINILM_V2` model setup from Lab 1.
