...
> **Screenshot update note:** The profile and runnable Media query-result captures have been refreshed from the Seer Media schema. The remaining provider-generated Select AI response captures should be refreshed after a provider-backed run, because their wording is generated at execution time.
...
Jessica, the DBA, has already configured a Select AI profile for the media schema. Nina can ask a question in ordinary language. Select AI uses the profile and the database metadata to generate SQL, run it, or explain the result.
...
>
> - A **natural-language prompt** is the question sent to Select AI, such as `Which five titles have the highest revenue?`
...
- Add the media tables that Select AI may use to the profile.
...
## Task 2: Add the media tables to the profile

The profile needs a list of tables that Select AI may use. Nina's questions require title, viewing-session, viewing-event, and viewer data, so Jessica adds those four tables to the `SEER_MEDIA_PROFILE` profile.

1. Add the media tables to the profile:
    ...
    BEGIN
      DBMS_CLOUD_AI.SET_ATTRIBUTE(
        ...
        attribute_value => '[{"owner": "' || USER || '", "name": "TITLES"}, {"owner": "' || USER || '", "name": "VIEWING_SESSIONS"}, {"owner": "' || USER || '", "name": "VIEWING_EVENTS"}, {"owner": "' || USER || '", "name": "VIEWERS"}]'
      );
    END;
    /
    </copy>
...
2. Confirm the object list:
    ...
    The result should list `TITLES`, `VIEWING_SESSIONS`, `VIEWING_EVENTS`, and `VIEWERS`. Select AI can now use these tables when it translates Nina's questions into SQL.
  
    ![task2](images/task2.png)

## Task 3: Ask a question and inspect the SQL

Nina starts with a simple question: which Midnight Harbor launch-weekend titles have the highest revenue per view? She first asks Select AI to show the SQL without running it.
...
1. Run the question with the `SEER_MEDIA_PROFILE` profile:
    ...
    SELECT DBMS_CLOUD_AI.GENERATE(
             prompt       => 'From the TITLES table, show the five titles with the highest revenue per view. Include title name, content type, genre, and revenue per view.',
             profile_name => 'SEER_MEDIA_PROFILE',
             action       => 'showsql'
           ) AS generated_sql;
    </copy>
...
2. Read the generated SQL before running it.
    ...
    Check whether the statement uses the expected title data, returns five rows, and orders `revenue_per_view` in a sensible way. Select AI can generate a valid-looking statement that does not answer the question precisely, so the generated SQL is part of the result Nina reviews.
...
2. Compare the answer with the SQL you inspected in Task 3.
    ...
    Select AI has generated and run SQL against the media schema. The query still runs under Nina's database privileges, and the result comes from the database tables rather than from a separate copy of the media data.
...
Nina's first question gives her a title ranking, but she also needs enough detail to decide what to review. She changes the question to request the content type, genre, distribution window, and revenue per view.
...
1. Use `showsql` to inspect this revised prompt:
    ...
    <copy>
    SELECT DBMS_CLOUD_AI.GENERATE(
             prompt       => 'From the TITLES table, show the five titles with the highest revenue per view. Include title name, content type, genre, distribution window, and revenue per view.',
             profile_name => 'SEER_MEDIA_PROFILE',
             action       => 'showsql'
           ) AS generated_sql;
    </copy>
...
This is the practical value of Select AI in Oracle AI Database. The question, generated SQL, and result stay connected to the governed media schema. Nina can ask in ordinary language, but she does not have to give up database access controls or the ability to inspect the query behind the answer.
