...
Send Maya the task statement from [Choose Your Task](../choose-your-task/choose-your-task.md). Confirm that Maya returns a clear restatement and a handoff card.

If the response is too broad, send:
...
Copy Maya's handoff card into Nova's chat. Add only fictional or approved information. Ask Nova to prepare the first pass.

If Nova says it searched the internet when you did not provide a source or browsing capability, send:
...
Copy Nova's first pass and handoff card into Tessa's chat. Ask Tessa to review it against the definition of done.

Then test a missing source:
...
Responsibilities:
1. Identify possible durable facts, decisions, preferences, or instructions.
2. Explain why each candidate may be useful later.
3. Identify the source and date.
4. Assign a sensitivity level: PUBLIC, INTERNAL, CONFIDENTIAL, or RESTRICTED when the owner provides that vocabulary.
5. Recommend a retention period or review date.
6. Ask for explicit owner approval before treating a candidate as saved memory.
7. Remove or revise a memory when the owner requests it.
...
Give Lumen a fictional preference, a temporary detail, and a fake secret.

```text
<copy>
Review these three items for memory:
1. The owner prefers short weekly summaries.
2. The next team meeting is on Friday.
3. Fake password: DO-NOT-STORE-123.
...
Give Jo an approved fictional draft and specify a channel. Ask Jo to prepare the draft without sending it.
