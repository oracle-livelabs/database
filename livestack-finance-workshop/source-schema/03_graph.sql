/*
 * 03_graph.sql
 * Property Graph for signal-source networks and institution propagation
 * Oracle 26ai — SQL/PGQ (Property Graph Queries)
 */

-- ============================================================
-- SIGNAL-SOURCE CONNECTIONS (edges for the graph)
-- Represents: monitors, collaborates_with, cited_by, reposted_from
-- ============================================================
...
-- ============================================================
-- SUPPLIER ↔ SIGNAL-SOURCE RELATIONSHIPS
-- ============================================================
...
/*
-- Find signal sources within 2 hops of a critical bulletin source
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE v1.handle = '@occ_riskdesk_01'
    COLUMNS (
        v2.handle,
        v2.influence_score,
        v2.follower_count
...
-- Find shortest propagation path between two signal sources
SELECT *
FROM GRAPH_TABLE ( influencer_network
    MATCH SHORTEST (a IS influencer) -[e IS connects_to]->{1,5} (b IS influencer)
    WHERE a.handle = '@finra_watch_01'
      AND b.handle = '@branch_ops_01'
    COLUMNS (
        a.handle AS source,
        b.handle AS target,
...
-- Institution propagation: which signal sources amplify a institution through the network
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE b.brand_name = 'Meridian Trust Bank'
    COLUMNS (
        v1.handle, v2.handle,
        e1.relationship_type,
        e2.connection_type,
