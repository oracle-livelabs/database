...
| ------------------- | ---------------------------------------------------------------------------------------------- |
| Business Problem    | Nina needs a telecommunications answer that can feed a subscriber-review screen.                            |
| Technical Challenge | The agent must use database data through an approved capability, not unrestricted access.      |
| Persona Focus       | You follow Nina as she turns a Select AI question into a small telecommunications assistant.              |
...
| Outcome             | Nina has a controlled agent that can answer questions from the telecommunications schema.                |

> **Prerequisite:** Complete [Lab 7: Ask Telecom Questions with Select AI](?lab=selectai). This lab uses the `GENAI` profile and its `object_list`.
...
1. Create the agent:
    ...
    BEGIN
      DBMS_CLOUD_AI_AGENT.CREATE_AGENT(
        ...
        attributes  => '{"profile_name": "genai", "role": "You are Nina Patel''s telecommunications data assistant. Answer questions using the approved SQL tool. Use database results for service plan, service order, monthly service charge, and subscriber facts. Do not invent values."}',
...
2. Create the task:
    ...
    BEGIN
      DBMS_CLOUD_AI_AGENT.CREATE_TASK(
        ...
        attributes => '{"instruction": "Answer Nina''s telecommunications question: {query}. Use NINA_TELECOM_SQL_TOOL once to retrieve the required data. Return a concise answer based on the database result. Do not repeat the same tool call and do not make changes to database records.", "tools": ["NINA_TELECOM_SQL_TOOL"], "enable_human_tool": "false"}',
...
1. Ask the agent:
    ...
    SELECT DBMS_CLOUD_AI_AGENT.RUN_TEAM(
             team_name   => 'NINA_TELECOM_TEAM',
             user_prompt => 'Which five service plans have the highest contracted monthly charges? Sum service_order_lines.line_total only for confirmed, active, or completed service_orders. Include the service plan name, category, total monthly charges, and connections ordered.',
    ...
    <!-- capture:CAP-21 -->
    > **Capture pending (CAP-21):** Add the SEER Telecomms result here after running this step in the manual database.
...
2. Review the answer.
    ...
    > **Note:** This team is instructed to answer without changing records. Instructions and descriptions are not a security boundary: LLUSER owns the tables. A production version needs a database principal with SELECT-only grants and appropriate row-level controls.
...
Open **AI-Assisted Service Assurance** to see the demo's agent question interface. The selected runtime at capture time was local `llama3.2`. This is an interface example, not evidence of `DBMS_CLOUD_AI_AGENT` execution. No agent question, intervention or data-changing action was submitted while taking the capture.
...
1. Review the latest team runs:
    ...
    <!-- capture:CAP-22 -->
    > **Capture pending (CAP-22):** Add the SEER Telecomms result here after running this step in the manual database.
...
2. Review the latest tool calls:
    ...
    <!-- capture:CAP-23 -->
    > **Capture pending (CAP-23):** Add the SEER Telecomms result here after running this step in the manual database.
