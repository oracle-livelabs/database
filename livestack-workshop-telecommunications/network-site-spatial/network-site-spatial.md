...
| --- | --- |
| Business Problem | Subscriber-service staff need nearby network sites to consider when arranging subscriber service reviews. |
| Technical Challenge | Moon needs to compare subscriber points with a region, then find the closest site for each subscriber. |
...
| Outcome | An operations user can see which subscribers need service in a region and which site is closest to each one. |
...
1. Run this query:
    ...
    <!-- capture:CAP-49 -->
    > **Capture pending (CAP-49):** Add the SEER Telecomms result here after running this step in the manual database.
...
The dataset contract assigns New York Network Region a synthetic demand index of `91` for the first routing review. Moon now measures the distance from each network-site point to the region boundary.
...
1. Run the distance query:
    ...
    <!-- capture:CAP-50 -->
    > **Capture pending (CAP-50):** Add the SEER Telecomms result here after running this step in the manual database.
    ...
    **Expected output: New York Service Coverage**
    ...
    Review the closest site and its distance. Rankings depend on the data your loader supplied. Verify the distances after loading the fixture.
...
2. Try another region.
    ...
    <!-- capture:CAP-51 -->
    > **Capture pending (CAP-51):** Add the SEER Telecomms result here after running this step in the manual database.


    

    The `unit` parameter controls the measurement unit. Review whether a site lies inside the Chicago Network Region polygon. A point inside or touching the polygon has distance zero. The contract assigns this region a synthetic demand index of 78; site rankings must be checked after loading.
...
1. Run the subscriber-to-site query:
    ...
    <!-- capture:CAP-52 -->
    > **Capture pending (CAP-52):** Add the SEER Telecomms result here after running this step in the manual database.
...
2. Review the result as an operations decision.
    ...
    Subscriber `LOCATION` is the service address supplied in the synthetic fixture. Each row gives a business user a subscriber to contact, the closest site, and the information needed to decide where the work should go. The result combines the region's demand score, subscriber details, site capacity, current load, and spatial distance in one SQL result.
...
3. Change the query to `Chicago Network Region`.
    ...
    Compare the subscribers and nearest sites with the New York result. The spatial predicates stay the same; only the region changes. This is the kind of query an operations dashboard can run when a business user selects a different demand region.
