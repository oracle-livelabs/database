/*
 * Seer Health Network Healthcare LiveStack - self-contained sandbox loader
 *
 * Run this one file with SQLcl while connected as ADMIN:
 *
 *   @healthcare-platform-handoff-loader.sql <lluser-password> <service-alias>
 *
 * The loader creates or resets LLUSER, grants its prerequisites, reconnects as
 * LLUSER, recreates every healthcare workshop object, loads the expanded and
 * deterministic dataset, builds vectors, graph and spatial artifacts, trains
 * the OML model, and prints validation results.
 *
...
BEGIN
  IF USER = 'ADMIN' THEN
    DBMS_OUTPUT.PUT_LINE('Running healthcare sandbox ADMIN prerequisites.');
  ELSE
    RAISE_APPLICATION_ERROR(
      -20001,
      'Start the healthcare platform loader as ADMIN. Current user: ' || USER
    );
  END IF;
END;
/
...
-- Generate the 5,000 signal vectors in deterministic, committed batches so a
-- sandbox provisioning job does not hold one long transaction while the model
-- processes the complete signal catalog.
DECLARE
  batch_first_id NUMBER;
  batch_last_id  NUMBER;
BEGIN
  FOR batch_number IN 0..19 LOOP
    batch_first_id := 101 + (batch_number * 250);
    batch_last_id := batch_first_id + 249;

    UPDATE hc_quality_signals
       SET signal_embedding =
             VECTOR_EMBEDDING(
               ADMIN.ALL_MINILM_L12_V2 USING signal_text AS DATA
             )
     WHERE signal_id BETWEEN batch_first_id AND batch_last_id;

    COMMIT;
    DBMS_OUTPUT.PUT_LINE(
      'Built signal vectors through signal ID ' || batch_last_id || '.'
    );
  END LOOP;
END;
/
...
WHERE model_name = 'CARE_DEMAND_RISK_MODEL';

PROMPT
PROMPT Healthcare sandbox release validation

COLUMN check_name FORMAT A30
COLUMN check_value FORMAT A20
COLUMN status FORMAT A8

WITH layer_counts (records) AS (
  SELECT COUNT(*) FROM hc_care_services
  UNION ALL
  SELECT COUNT(*) FROM hc_quality_signals
  UNION ALL
  SELECT COUNT(*) FROM hc_service_requests
  UNION ALL
  SELECT COUNT(*) FROM hc_care_services WHERE service_embedding IS NOT NULL
  UNION ALL
  SELECT COUNT(*) FROM hc_quality_signals WHERE signal_embedding IS NOT NULL
  UNION ALL
  SELECT COUNT(*) FROM hc_semantic_matches
), release_checks (display_order, check_name, check_value, expected_value) AS (
  SELECT 1, 'CONNECTED_USER', USER, 'LLUSER' FROM dual
  UNION ALL
  SELECT 2, 'TOTAL_TRACKED_RECORDS', TO_CHAR(SUM(records)), '14796'
  FROM layer_counts
  UNION ALL
  SELECT 3, 'CARE_SITES', TO_CHAR(COUNT(*)), '2000' FROM hc_care_sites
  UNION ALL
  SELECT 4, 'LOGISTICS_SITES', TO_CHAR(COUNT(*)), '30' FROM hc_logistics_sites
  UNION ALL
  SELECT 5, 'CARE_DEMAND_RISK_MODEL', TO_CHAR(COUNT(*)), '1'
  FROM user_mining_models
  WHERE model_name = 'CARE_DEMAND_RISK_MODEL'
  UNION ALL
  SELECT 6, 'INVALID_HEALTHCARE_OBJECTS', TO_CHAR(COUNT(*)), '0'
  FROM user_objects
  WHERE status = 'INVALID'
    AND (
      object_name LIKE 'HC\_%' ESCAPE '\'
...
SELECT check_name,
       check_value,
       CASE WHEN check_value = expected_value THEN 'PASS' ELSE 'FAIL' END AS status
FROM release_checks
