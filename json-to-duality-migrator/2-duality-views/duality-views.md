...
## Task 1: Create a duality view and load some data
...
1. Create the attendees and sessions tables. We also need to create a mapping table between attendees and sessions (map\_attendees\_to\_sessions tables) to model the many-to-many relationship between them.
...
2. Add a field to an existing document. We will add a country field to Julian's attendee document to specify his country of origin. This field gets stored in the flex JSON column since it is not mapped to any column in the attendees table.
...
2. Update the session name in only attendee id 1's document using JSON_TRANSFORM to update the session name only for the matching session id.
   ...
   This statement updates one documents, as a result of which the row in the session table storing session id 40's information has been updated. This should automatically update session information in all other documents containing it.
...
3. Select all documents from the view to see the updated documents.
   ...
   We can see that the session name for session id 40 has now been updated consistently everywhere. Updating the data in one place updates it consistently everywhere - this is one of the biggest advantages of using duality views.
